...
if st.button("Analyze"):
    if text.strip() == "":
        ...
        st.subheader("🌐 Translated Text (used for AI)")
        st.info(translated_text)

        # STEP 4: RUN MODELS ON TRANSLATED TEXT
        ...
        st.subheader("⚖️ Sexism Detection")
        ...
        if combined_score > 0.3 or (keyword_flag and combined_score > 0.2):
            st.error("⚠️ Potentially Sexist Content")
        elif tox["toxic"] > 0.5:
